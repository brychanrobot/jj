...
where
    ...
    NI: IntoIterator<Item = T>,
{
    let neighbors_fn = move |node: &T| Ok(neighbors_fn(node).into_iter().collect_vec());
...
/// the error.
pub fn closest_common_node_ok<T, ID, E, II1, II2>(
    set1: II1,
    set2: II2,
    id_fn: impl Fn(&T) -> ID,
    mut neighbors_fn: impl FnMut(&T) -> Result<Vec<T>, E>,
) -> Result<Option<T>, E>
where
    ID: Hash + Eq,
